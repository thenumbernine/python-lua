...
-- static methos
function wrapper._Py_IsImmortal(op)
    op = ffi.cast('PyObject*', op)
    return ffi.cast('int32_t', op[0].ob_refcnt) < 0
end
function wrapper.Py_DECREF(op)
    op = ffi.cast('PyObject*', op)
    if wrapper._Py_IsImmortal(op) then
        return
    end
    op[0].ob_refcnt = op[0].ob_refcnt - 1
	if op[0].ob_refcnt == 0 then
        lib._Py_Dealloc(op)
    end
end
function wrapper.Py_XDECREF(op)
    op = ffi.cast('PyObject*', op)
	if op ~= ffi.null then
